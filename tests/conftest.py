...
@pytest.fixture
async def role_pg_client(engine):
    pg = postgres.RolePostgresClient(engine)
    yield pg
    pg.engine.close()
    await pg.engine.wait_closed()
...
@pytest.fixture
async def db(loop, engine, user_pg_client, role_pg_client):
    ...
    await db_setup.setup_db(role_pg_client, user_pg_client)
    yield
